...
class TuyaDevice {
    constructor(deviceInfo) {
        ...
        this.useDeviceTopic = deviceInfo.useDeviceTopic;
        ...
        if (this.config.name) { 
            this.options.name = this.config.name.toLowerCase().replace(/\s|\+|#|\//g,'_');
        }
        ...
        if(this.useDeviceTopic) {
            ...
            if (this.options.name) {
                this.baseTopic = this.baseTopic.replace('%device%', this.options.name);
            } else {
                this.baseTopic = this.baseTopic.replace('%device%', this.options.id);
            }
        } else {
            if (this.options.name) {
		...
		debug('Device path: ' + this.baseTopic);
        ...
        this.device.on('connected', async () => {
            ...
            if (this.device.isConnected()) {
                ...
                this.heartbeatsMissed = 0;
                this.publishMqtt(this.baseTopic + 'LWT', 'online');
                this.init();
            }
        })
        ...
        this.device.on('disconnected', async () => {
            ...
            this.publishMqtt(this.baseTopic + 'LWT', 'offline');
    ...
    async republish() {
        ...
        this.publishMqtt(this.baseTopic+'LWT', status);
        await utils.sleep(1);
        this.init();
    }
